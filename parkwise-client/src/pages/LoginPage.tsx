...
import { LogIn, Mail, Lock, ArrowRight, AlertCircle, Car, ShieldCheck } from 'lucide-react';
import { AppUser, getOAuthStartUrl, loginUser, OAuthProvider, saveSession } from '../lib/api';
...
const LoginPage = ({ onAuthSuccess }: { onAuthSuccess?: (user: AppUser) => void }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.user.role === 'sys_admin') navigate('/sysadmin/dashboard');
      else if (response.user.role === 'parking_admin') navigate('/admin/dashboard');
  ...
  const socialLogin = (provider: OAuthProvider) => {
    window.location.assign(getOAuthStartUrl(provider, { mode: 'login', role: 'driver' }));
  };

  return (
    <div className="max-w-6xl mx-auto pt-10 px-4 pb-20">
      <div className="flex flex-col lg:flex-row gap-12 items-start">
        {/* Left Side: Note with Picture */}
        <motion.div
          initial={{ opacity: 0, x: -20 }}
          animate={{ opacity: 1, x: 0 }}
          className="hidden lg:block w-full lg:w-1/2 space-y-8"
        >
          <div className="relative rounded-[3rem] overflow-hidden shadow-2xl border-8 border-white aspect-[4/5]">
            ...
            <div className="absolute bottom-10 left-10 right-10 text-white space-y-4">
              ...
              <h2 className="text-3xl font-bold leading-tight">
                Welcome back to <br /> smarter parking.
              </h2>
              <p className="text-gray-200 text-lg">
                "Finding parking used to be a nightmare. Now it's the easiest part of my day."
              </p>
              <div className="flex items-center space-x-3 pt-4">
                <div className="h-12 w-12 rounded-full bg-orange-100 border-2 border-white overflow-hidden">
                  <img src="https://i.pravatar.cc/150?u=parkwise_login" alt="User" />
                </div>
                <div>
                  <p className="font-bold">Sara Tadesse</p>
          ...
          <div className="bg-orange-50 p-8 rounded-[2.5rem] border border-orange-100 space-y-4">
            ...
            <p className="text-orange-800 font-medium leading-relaxed">
              Your account is protected by multi-factor authentication and real-time monitoring to keep your data safe.
            </p>
          </div>
        </motion.div>
        ...
        >
          <div className="space-y-4 mb-10">
            ...
            <p className="text-gray-500 text-lg">Sign in to manage your parking</p>
          </div>
          ...
          <div className="space-y-4">
            ...
            >
              ...
              <span className="font-bold text-gray-600 group-hover:text-orange-600">Sign in with Google</span>
            </button>
            <button
              type="button"
            ...
            >
              ...
              <span className="font-bold text-gray-600 group-hover:text-orange-600">Sign in with Facebook</span>
            </button>
            <button
              type="button"
            ...
            >
              ...
              <span className="font-bold text-gray-600 group-hover:text-orange-600">Sign in with GitHub</span>
            </button>
          </div>
